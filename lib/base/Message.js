...
const { writeExifWebp } = require("../functions/sticker");
...
class Message extends Base {
  ...
  async sendMessage(
    jid,
    content,
    opt = { packname: config.PACKNAME, author: config.AUTHOR , fileName: "WhatsBixby" },
    type = "text"
  ) {
  ...
  async UpdateLocal(message) {
    async function checkAndInitializeGitRepo() {
      ...
      if (!isGitRepo) {
          try {
              await git.init();
              await git.addRemote('origin', 'https://github.com/DX-MODS/WhatsBixby.git');
          } catch (error) {
